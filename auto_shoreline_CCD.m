...
%June 2018
%Modified by Patrick Scordato, WHOI SSF 2018- United States Geological
%Survey
%Date of modification: 07/27/18
...
%Make new directories to store mat and text files and define your file name
new_dir_mat= mkdir('D:\Scordato_SSF_2018\Projects\SandwichBeachCam\shoreline_mapping\SL_coordinates\Dune_Tests\January_04_2018\mat_files\')
file_name= 'D:\Scordato_SSF_2018\Projects\SandwichBeachCam\shoreline_mapping\SL_coordinates\Dune_Tests\January_04_2018\mat_files\'
new_dir_txt = mkdir('D:\Scordato_SSF_2018\Projects\SandwichBeachCam\shoreline_mapping\SL_coordinates\Dune_Tests\January_04_2018\txt_files\');
file_name2= 'D:\Scordato_SSF_2018\Projects\SandwichBeachCam\shoreline_mapping\SL_coordinates\Dune_Tests\January_04_2018\txt_files\';

%Import your rectified image pathways
imagePn= 'D:\Scordato_SSF_2018\Projects\SandwichBeachCam\images\frameRect_Files\Survey_Aligned_Test_Images\March_30_2016\'
d= dir(imagePn);
...
for ii= 3: length(d)
    inputFn= fullfile(d(ii).folder, d(ii).name)
    load(inputFn, 'frameRect')
    load(inputFn, 'wl') 
    ...
    load('D:\Scordato_SSF_2018\Source_Code\Shoreline_Extraction\x_coordinates');
    x_coordinates= (x_coordinates);

    %Import your y coordinates
    load('D:\Scordato_SSF_2018\Source_Code\Shoreline_Extraction\y_coordinates')
    ...
    thresh_weightings = [1/3 2/3]; %[1/3 2/3] %%  %This weights the authomatic thresholding towards the sand peak (works well in SE Australia)
    ...
    sl.e = NaN(1,length(x_coordinates));
    sl.n=  NaN(1,length(y_coordinates));
    sl.elev= NaN(1, length(x_coordinates));
    warning off
    for i = 1:length(x_coordinates)
        ...
        max_distance = sqrt(diff(y_coordinates(:,i))^2+ diff(x_coordinates(:,i))^2);
            ...
            if ~isempty(I)
                ...
                %Calculate the shoreline coordinates in UTM coordinates 
                sl.e(i)= sl.x(i)+ 376523.828 ;
                sl.n(i) = sl.y(i)+ 4625139.430 ;
                sl.elev(i)= wl
    ...
    if editoption ==1
        ...
        sl.threshold = thresh;
         ...
         %Save the all the shoreline coordinates as a mat file with differing water level 
         data_name= d(ii).name ;
         outPn= strcat(file_name, data_name) ;
         save(outPn, 'sl') ;

        %Save to text file
        data_name_txt= replace(data_name, 'mat', 'txt'); %d(:).name ;
        outPn_txt= strcat(file_name2, data_name_txt) ;

        format long g
        Easting= transpose(sl.e);
        Northing= transpose(sl.n);
        Elevation= transpose(sl.elev) ;
        east_north_ele= [ Easting Northing Elevation];
        save(outPn_txt, 'east_north_ele', '-ASCII', '-append');
